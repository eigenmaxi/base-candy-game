...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function playSound(name) {
    ...
    try {
      ...
      if (sound) {
        sound.play().catch(e => console.log('Audio play failed (user interaction may be required):', e));
      }
    } catch(e) {
  ...
  function startBackgroundMusic() {
    ...
    try {
      bgMusic.volume = 0.2;
      bgMusic.loop = true;
      bgMusic.play().catch(e => console.log('Background music play failed (user interaction may be required):', e));
    } catch(e) {
  ...
  async function connectWallet() {
    try {
      ...
      // Create ethers provider using v6 syntax
  ...
  async function shareToFarcaster() {
    try {
      ...
      // Create share text without URL (URL will be embedded separately)
      const shareText = `🍬 I just scored ${score} points in Base Candy!
